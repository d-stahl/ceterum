-- Bug B: When all players have locked demagog placements, advance_round skipped
-- demagogery entirely (phase → demagogery_resolved). This bypassed resolve_demagogery,
-- so locked demagogs earned no influence and were never released.
-- Fix: Remove the skip. Let demagogery phase run normally — the client will auto-submit
-- sit-out placements for players with all slots locked, triggering normal resolution.
...
BEGIN
  ...
  -- Carry forward only fresh (non-locked) demagog placements as locked for the new round.
  -- Already-locked demagogs have served their extra round and expire here.
  ...
  -- No skip logic — demagogery always runs. Players with all locked slots
  -- will auto-submit sit-out placements from the client.
