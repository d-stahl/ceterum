-- Non-vote controversies (endeavour, clash, schism) skip the SL declaration step.
-- They should start as 'voting' instead of 'declared'.

-- Fix submit_senate_leader_actions: set initial status based on controversy type
...
BEGIN
  ...
  v_first_status := CASE WHEN v_first_type = 'vote' THEN 'declared' ELSE 'voting' END;
...
-- Fix advance_controversy_phase: set status based on controversy type
...
BEGIN
  IF p_current_phase = 'ruling_voting_1' THEN
    ...
    v_status := CASE WHEN v_type = 'vote' THEN 'declared' ELSE 'voting' END;
