...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const slDeclaredSideKey = csState.schism_declared_side as string;
    const teamMemberIds = csState.schism_team_members as string[];
    ...
    const controversy = snapRes.data.snapshot as SchismControversy;
    const submissions = buildSchismSubmissionsFromDb(subsRes.data ?? []);
    ...
    const result = resolveSchism(submissions, controversy.schismConfig, slDeclaredSideKey, teamMemberIds);
    ...
    if (Object.keys(result.axisEffects).length > 0) {
      // Supporters "backed" the SL's side; if sabotaged, they backed the losing side
      const pseudoVotes = result.supporters.map((pid) => ({
        playerId: pid,
        resolutionKey: '_schism_support',
        influenceSpent: 1,
      }));

      // Saboteurs backed the other side
      const saboteurVotes = result.saboteurs.map((pid) => ({
        playerId: pid,
        resolutionKey: '_schism_sabotage',
        influenceSpent: 1,
      }));

      // For affinity: compute based on the winning side's axis effects
      // Supporters and saboteurs both get affinity effects proportional to their alignment
      const allVotes = [...pseudoVotes, ...saboteurVotes];
      const winningResolution = result.wasSabotaged ? '_schism_sabotage' : '_schism_support';
    ...
    if (winningSide.followUpKey) {
      await adminClient
        .from('game_follow_up_pool')
        .upsert({
          game_id,
          controversy_key: winningSide.followUpKey,
          unlocked_at_round: round.round_number,
          used: false,
        }, { onConflict: 'game_id,controversy_key' });
      const followUpDef = CONTROVERSY_MAP[winningSide.followUpKey];
      if (followUpDef) {
        await adminClient
          .from('game_controversy_snapshots')
          .upsert({
